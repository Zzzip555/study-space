...
void EchoServer::onConnection(const TcpConnectionPtr& conn)
{
  ...
  if (conn->connected())
  {
    EntryPtr entry(new Entry(conn));//临时变量
    connectionBuckets_.back().insert(entry);//插入之后引用计数为2
    dumpConnectionBuckets();
    WeakEntryPtr weakEntry(entry);
    conn->setContext(weakEntry); //保存到context中是因为在OnMessage还有用到entry
  }                              //用弱引用是为了不增加entry的引用计数
  else
  {
