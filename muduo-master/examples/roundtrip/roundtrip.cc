                           ...
                           muduo::Timestamp receiveTime)
{
  ...
  {
    memcpy(message, buffer->peek(), frameLen);//message[0]是客户端发送时间
    buffer->retrieve(frameLen);
    message[1] = receiveTime.microSecondsSinceEpoch(); //message[1]是服务端接受时间
    conn->send(message, sizeof message); //发送给客户端
  }
}
                           ...
                           muduo::Timestamp receiveTime)
{
  ...
  while (buffer->readableBytes() >= frameLen)
  {
    ...
    buffer->retrieve(frameLen);
    int64_t send = message[0];  //客户端发送时刻
    int64_t their = message[1]; //服务端接收时刻
    int64_t back = receiveTime.microSecondsSinceEpoch(); //客户端接受时刻
    int64_t mine = (back+send)/2;
    LOG_INFO << "round trip " << back - send
             << " clock error " << their - mine;//时钟偏差
  }
}
